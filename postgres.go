...
func (b *postgreSQLBackend) InsertRows(ctx context.Context, op insertOp) (insertRes, error) {
	if len(op.rows) == 0 {
		return nil, nil
	}
...
func (b *postgreSQLBackend) UpdateRows(ctx context.Context, op updateOp) (updateRes, error) {
	if len(op.rows) == 0 {
		return nil, nil
	}
